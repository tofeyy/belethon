...
async def _format_quote(event: types.Message, reply=None,
    ...
    if reply and reply.raw_text:
        reply = {
            "name": get_display_name(reply.sender) or "حساب محذوف",
            "text": reply.raw_text,
...
async def create_quotly(event, url="https://bot.lyo.su/quote/generate",
    ...
    try:
        request = await async_search(url, method="POST", json=content, re_json=True)
